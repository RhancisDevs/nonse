...
local bttn = frame.ScrollingFrame:GetChildren()[5].Frame:GetChildren()[7].TextButton
...
local USE_ONLY_SEED = "50"
local CHECK_FRUIT_MIN_KG = 40
...
local plantedSeeds = 0

local SeedLimit = nil
if tostring(USE_ONLY_SEED):lower() ~= "all" then
    SeedLimit = tonumber(USE_ONLY_SEED)
end
...
while State.Running do
	...
	if ok then
        ...
        if SeedLimit and plantedSeeds >= SeedLimit then
            handleSeedDepleted(plot)
            break
        end
